...
void Earth::setupGeometry() {
    ...
    for(float lat=0; lat<pi(); lat+=pi()/STACKS){
        for(float lon=0; lon<=2*pi(); lon+=pi()/SLICES){
            Vector3 location = getPosition(lat,lon);
            cpuVerts.append(location);
            cpuNorms.append(location.unit());
            cpuTexCoords.append(Vector2(1-(((SLICES/pi())*(lon))/(2*SLICES)),(((STACKS/pi())*(lat))/(STACKS))));
        }
    }
    for(float lon=0; lon<=2*pi(); lon+=pi()/SLICES){
        ...
        cpuVerts.append(location);
        cpuNorms.append(location.unit());
        cpuTexCoords.append(Vector2(1-(((SLICES/pi())*(lon))/(2*SLICES)),(((STACKS/pi())*(pi()))/(STACKS))));
    }
...
Vector3 Earth::getPosition(float latitude, float longitude) {
